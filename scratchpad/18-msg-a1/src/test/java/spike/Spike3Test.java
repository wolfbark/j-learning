...
class Spike3Test {
    ...
    static Map<String, Object> shareProps(String group) {
        var props = base();
    ...
    static void resetEarliest(String group) throws Exception {
        try (Admin admin = Admin.create(base())) {
    ...
    @Test
    void classicStableExactlyOneIdle() throws Exception {
        String topic = "spike3-classic";
        String group = "spike3-classic-group";
        createTopic(topic, 3);
        produce(topic, 30);
        ...
        while (System.currentTimeMillis() < deadline) {
            ...
            long withOne = consumers.stream().filter(c -> c.assignment().size() == 1).count();
            long empty = consumers.stream().filter(c -> c.assignment().isEmpty()).count();
            if (withOne == 3 && empty == 1) {
                break;
            }
        }
        System.out.println("=== CLASSIC settle_ms=" + (System.currentTimeMillis() - t0));
    ...
    @Test
    void shareGroupAssignmentAndFairness() throws Exception {
        String topic = "spike3-share";
        String group = "spike3-share-group";
        createTopic(topic, 3);
        resetEarliest(group);
        ...
        var joined = new CountDownLatch(4);
        ...
        for (int i = 0; i < 4; i++) {
            ...
            threads.add(Thread.ofPlatform().start(() -> {
                try (var c = new KafkaShareConsumer<String, String>(shareProps(group))) {
                    c.subscribe(List.of(topic));
                    c.poll(Duration.ofMillis(1000));   // establish membership before work arrives
                    joined.countDown();
                    long deadline = System.currentTimeMillis() + 60_000;
        ...
        try (Admin admin = Admin.create(base())) {
            ...
            System.out.println("=== SHARE group state=" + desc.groupState() + " members=" + desc.members().size());
        ...
        long working = counts.values().stream().filter(v -> v.get() > 0).count();
        System.out.println("=== SHARE consumers_with_work=" + working);
    }

    @Test
    void shareGroupReleaseRedelivers() throws Exception {
        String topic = "spike3-release";
        String group = "spike3-release-group";
        createTopic(topic, 1);
        resetEarliest(group);
        produce(topic, 1);
        long t0 = System.currentTimeMillis();
        try (var c = new KafkaShareConsumer<String, String>(shareProps(group))) {
            ...
            while (System.currentTimeMillis() < deadline && deliveries < 3) {
                var recs = c.poll(Duration.ofMillis(500));
                for (var r : recs) {
                    deliveries++;
                    System.out.println("  delivery #" + deliveries + " count=" + r.deliveryCount()
                            + " offset=" + r.offset());
                    c.acknowledge(r, deliveries < 3 ? AcknowledgeType.RELEASE : AcknowledgeType.ACCEPT);
                }
                if (recs.count() > 0) {
                    c.commitSync();
                }
            }
            System.out.println("=== RELEASE deliveries=" + deliveries + " ms=" + (System.currentTimeMillis() - t0));
        }
    }
}
