...
class Checkpoint6PoolDiagnosisTest extends PostgresTestBase {
    ...
    void littlesLaw() {
        meters.getMeters().stream().map(m -> m.getId().getName()).filter(n -> n.startsWith("hikari")).distinct().sorted().forEach(n -> System.out.println("METER " + n));
        meters.getMeters().stream().map(m -> m.getId().getName()).filter(n -> n.contains("connections")).distinct().sorted().forEach(n -> System.out.println("METER " + n));
