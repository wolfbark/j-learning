...
/**
 * Retrieval-augmented generation, in full, with nothing hidden behind an advisor: retrieve,
 * assemble, generate.
 */
@Service
public class RagService {
    ...
    /** Injected and unused: step 5 offers it to the model with {@code .tools(lessonIndex)}. */
    ...
    public Answer ask(String question) {
        ...
        return new Answer(content, passages.stream().map(Passage::toCitation).toList(),
                confidenceOf(passages));
    }
    ...
    /**
     * The prompt. Note the passage numbering and the {@code source=}/{@code heading=} labels: a
     * model can only cite what you told it, in the vocabulary you told it in.
     */
